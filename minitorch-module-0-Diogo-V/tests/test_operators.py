...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert 0 <= sigmoid(a) <= 1, f"Needs to be between 0 and 1. Got {sigmoid(a)}."
    assert_close(1 - sigmoid(a), sigmoid(-a))
    assert 0.5 == sigmoid(0), "Sigmoid of 0.5 should be zero."
    assert sigmoid(1) < sigmoid(2) < sigmoid(3), "Sigmoid is strictly increasing."
...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    assert mul(a, b) == mul(b, a), "Multiplication is symmetric."
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert_close(mul(a, b + c), add(a * b, a * c))
...
def test_other(a: float) -> None:
    ...
    assert id(a) == a, "Input is correct"
    assert neg(id(a)) == -a, "Negative is correct"
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sum_ls1 = minitorch.operators.sum(ls1)
    sum_ls2 = minitorch.operators.sum(ls2)
    sub_sums = sum_ls1 + sum_ls2
    list_sum = minitorch.operators.sum(addLists(ls1, ls2))
    assert_close(sub_sums, list_sum)
