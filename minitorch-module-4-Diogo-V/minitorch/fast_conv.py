        ...
        for in_channel in prange(in_channels):
            for k in range(kw):
                if reverse:
                    k = kw - k - 1
                w = weight[
                    out_idx[1] * weight_strides[0]
                    + in_channel * weight_strides[1]
                    + k * weight_strides[2]
                ]

                in_val = 0
                common_in_idx_prefix = (
                    out_idx[0] * input_strides[0] + in_channel * input_strides[1]
                )
                if reverse:
                    reverse_offset = out_idx[2] - k
                    if reverse_offset >= 0:
                        in_val = input[
                            common_in_idx_prefix + reverse_offset * input_strides[2]
                        ]
                else:
                    forward_offset = k + out_idx[2]
                    if forward_offset < width:
                        in_val = input[
                            common_in_idx_prefix + forward_offset * input_strides[2]
                        ]
                total += w * in_val
